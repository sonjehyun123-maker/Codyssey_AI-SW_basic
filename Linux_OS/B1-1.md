...
# 2. 기본 정책 설정
sudo ufw default deny incoming # 외부->내부 차단
sudo ufw default allow outgoing # 내부->외부 허용
...
# tcp   LISTEN 0      5      0.0.0.0:15034      0.0.0.0:*    users:(("python3",pid=10119,fd=3))
```
...
if ! ss -tuln | grep LISTEN | grep -q ":$PORT "; then. #"15034 포트가 실제 LISTEN 상태냐?"
...
```text
[2026-05-13 19:27:01] PID:10119 CPU:1.1% MEM:7.2% DISK_USED:1%
[2026-05-13 19:28:02] PID:10119 CPU:1.1% MEM:7.2% DISK_USED:1%
...
확인 순서:
1. ss -tulnp 로 LISTEN 상태 확인    //포트 확인
2. application log 확인            // 애플리케이션 내부 실패 원인 확인
3. ps/pgrep 로 process 상태 확인    // 프로세스가 멈춰 있는 가능성 확인
4. journalctl/system log 확인      // 애플리케이션 바깥의 시스템 문제 확인 EX) 메모리 부족, 보안 차단 등
5. firewall(UFW) 상태 확인          // 네트워크 정책 차단 확인
```
