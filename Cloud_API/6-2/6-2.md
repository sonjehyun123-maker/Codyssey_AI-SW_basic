...
- `mask_sensitive()`는 20자 이상 영숫자 문자열(API Key 패턴)과 이메일 패턴을 정규식으로 치환합니다. `safe_mode=True`일 때만 적용되며, 두 프롬프트 함수가 공통으로 호출합니다.
...
- **민감정보 대응**: `-safe-mode` 옵션 사용 시, diff 전송 전 `mask_sensitive()`가 20자 이상 영숫자 문자열(Key 형태)과 이메일 패턴을 `[MASKED_KEY]` / `[MASKED_EMAIL]`로 치환합니다. 완벽한 탐지를 보장하지는 않으므로, 실제 민감정보가 포함된 저장소에서는 diff 내용을 직접 확인 후 사용을 권장합니다.
